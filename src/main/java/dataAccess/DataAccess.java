...
public class DataAccess {
	...
	public boolean gertaerakSortu(String description, Date eventDate, String sport) {
		...
		if (db.getTransaction().isActive())
			db.getTransaction().commit(); // Esta línea evita errores
		db.getTransaction().begin();
	...
	private void forJarraitzaileak(Vector<Quote> q2, Double balioa, Integer apustuBikoitzaGalarazi, Registered user,
	...
	public boolean gertaeraEzabatu(Event ev) {
		...
		resultB = comprobarRespuestasPreguntas(resultB, listQ);
		if (resultB == false) {
			return false;
	...
	private void recorrerListaQuotes(List<Quote> listQUO) {
		for (int j = 0; j < listQUO.size(); j++) {
			Quote quo = db.find(Quote.class, listQUO.get(j));
			obtenerApuestasPorQuote(quo);
		}
	}

	private void obtenerApuestasPorQuote(Quote quo) {
	...
	private boolean comprobarRespuestasPreguntas(boolean resultB, List<Question> listQ) {
